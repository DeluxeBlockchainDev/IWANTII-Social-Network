...
function CountdownTable(props) {
  ...
  const handleUserInput = value => {
    // Show all item first
    const { countdowns, defaultPerPage } = tableState;
    if (value !== '') {
      setTableState({
        ...tableState,
        rowsPerPage: countdowns
      });
    } else {
      setTableState({
        ...tableState,
  ...
  const renderCell = (dataArray, keyArray) => keyArray.map((itemCell, index) => (
    ...
    >
      {itemCell.id === 'action' ? (
        ...
        moment(dataArray[itemCell.id]).format('DD/MM/YYYY')
      ) : itemCell.id === 'winner' ? (
        users.length > 0 ? (
        ...
        requests.find(item => item._id === dataArray[itemCell.id]) ? (
          requests.find(item => item._id === dataArray[itemCell.id]).text
        ) : (
          ''
        )
      ) : (
  ...
  return (
    <Fragment>
      ...
      <AlertDialog
        ...
        text="Do you really want to remove this countdown?"
...
CountdownTable.defaultProps = {
  loading: false
};

export default withStyles(styles)(CountdownTable);
