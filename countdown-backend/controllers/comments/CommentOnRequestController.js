...
const Comment = require("../../models/comments/CommentOnRequestModel");
...
/**
 * Get all comments by request id
 */
exports.CommentsById = [
	function(req, res) {
		try {
			Comment.find({ request: req.params.id }, function(err, comments) {
				// return count only
				if (req.query.count) {
	...
	async function(req, res) {
		...
		try {
			...
			if (request === null) {
				return res
					.status(400)
					.json("request id " + req.body.request + " does not exist.");
			}
		} catch (err) {
	...
	async function(req, res) {
		...
		try {
			Comment.findById(req.params.commentId, async function(err, found) {
				...
				if (
					found.request.toString() !== req.body.request ||
