const Comment = require("../../models/comments/CommentOnUserModel");
...
exports.CommentsById = [
	function(req, res) {
		try {
			Comment.find({ winner: req.params.id }, function(err, comments) {
				// return count only
				if (req.query.count) {
	...
	async function(req, res) {
		...
		try {
			...
			if (winner === null) {
				return res
					.status(400)
					.json("user id " + req.body.winner + " does not exist.");
			}
		} catch (err) {
			return res.status(400).json("user does not exist.");
		}
	...
	async function(req, res) {
		...
		try {
			Comment.findById(req.params.commentId, async function(err, found) {
				...
				if (
					found.winner.toString() !== req.body.winner ||
